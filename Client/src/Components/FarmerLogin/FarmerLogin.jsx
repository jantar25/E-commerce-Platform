...
const FarmerLogin = () => {
    ...
    const {isFetching} = useSelector((state)=> state.farmer)
        ...
        {signIn? (
             ...
             <div className='flex flex-col md:flex-row justify-between items-center'>
                 <div className='flex-1 flex flex-col my-4 md:mr-16 w-full text-white'>
                     <input placeholder='Email' onChange={(e)=> setEmail(e.target.value)} className='mb-2 px-4 py-2 bg-[#232B2B] rounded'/>
                     <input placeholder='Password' type="password" onChange={(e)=> setPassword(e.target.value)} className='px-4 py-2 bg-[#232B2B] rounded'/>
                 </div>
                 <div className='flex flex-col items-center justity-center'>
                     <button className='bg-black text-white py-2 px-8 text-lg font-[700] rounded-md' onClick={handleClick}>
